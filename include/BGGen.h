...
/** \brief Possible Planet Types that can be drawn
 *
 * This enum declares the possible types of planets that can be drawn in the background
 * generated by bggen. Mostly just the coloring style.
 */
enum PLANET_TYPE{
    ICE,         //!< Blue Radial Gradient Planet
    ROCKY,       //!< All Rock
    EARTH_LIKE,  //!< Like Earth with land and water
    WATER        //!< A Water planet colored blueish-green
};
...
private:
    ...
    /**\brief draws a one pixel wide stripe that follows the coloring rules as defined
     *        in the PLANET_TYPE enum.
     * \param t The planet type
     * \param x0 Left Side on the x-axisit
     * \param y0 y Pos
     * \param width length of the strip
     * \note  This function can only draw horizontal stripes. NOT VERTICAL.
     */
    void drawPlanetStripe(PLANET_TYPE t, uint32_t x0, uint32_t y0, uint32_t width);

    /**\brief Draws a random number of stars at random positions and of varying intensity
     */
    void drawStars();
